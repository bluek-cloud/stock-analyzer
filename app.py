...
st.markdown("""
    ...
    .zoom-ctrl { display: flex; gap: 5px; margin-bottom: 10px; }
    </style>
...
def generate_detailed_opinions(df, sup, res, currency, decimals, is_short_term, time_unit):
    ...
    if rsi >= 70: comments['RSI'] += "이는 시장의 탐욕이 극에 달해 단기 고점 신호를 보내는 것으로, 신규 진입보다는 보유 물량의 수익 실현을 우선적으로 고려해야 하는 자리입니다."
    elif rsi <= 30: comments['RSI'] += "매도세가 과도하게 쏠린 공포 구간입니다. 역사적으로 기술적 반등이 강하게 나타나는 타점이며, 분할 매수 관점에서 매력적인 진입 기회로 분석됩니다."
    else: comments['RSI'] += "현재 매수와 매도 세력이 팽팽하게 맞서고 있습니다. 뚜렷한 과열이나 침체 없이 기존 추세를 유지하거나 박스권 흐름을 이어갈 가능성이 큽니다."
    ...
    if macd > signal: comments['MACD'] += "단기 이동평균선이 장기선을 뚫고 올라온 상태로, 주가 상승에 가속도가 붙는 모멘텀 확장 구간입니다. 추세 추종 전략이 유효합니다."
    else: comments['MACD'] += "단기 모멘텀이 둔화되면서 하락 압력이 우세한 상황입니다. 주가가 지지선을 지지하지 못할 경우 조정 폭이 깊어질 수 있으니 보수적인 관망이 필요합니다."

    # 거래량 분석
    comments['VOL'] = f"상대 거래량이 최근 5{time_unit} 평균 대비 **{vol_ratio:.0f}%** 수준입니다. "
    if vol_ratio > 150: comments['VOL'] += "평소보다 압도적인 대량 거래가 수반되고 있습니다. 이는 세력의 매집이나 대형 재료가 반영된 결과로, 조만간 변동성이 극대화될 확률이 매우 높습니다."
    else: comments['VOL'] += "평이한 수준의 거래가 이뤄지고 있습니다. 강력한 돌파나 하락보다는 현재 가격대를 다지는 횡보 장세가 예상됩니다."
    ...
    if obv > prev_obv: comments['OBV'] += "주가의 움직임보다 앞서 수급이 개선되고 있는 긍정적인 신호입니다. '숨은 매집' 단계일 가능성이 큽니다."
    else: comments['OBV'] += "주가가 버티더라도 스마트 머니는 이미 빠져나가는 중일 수 있습니다. 추세 붕괴 전조 현상이니 주의를 요합니다."

    # ATR 및 전략
    vol_pct = (atr / close) * 100
    comments['ATR'] = f"현재 {time_unit}당 평균 **{vol_pct:.1f}% ({atr:,.{decimals}f}{currency})**의 변동폭을 보입니다. "
    comments['ATR'] += f"이 변동폭은 손절가 설정의 기준이 됩니다. 현재 변동성을 고려할 때, 예상치 못한 노이즈를 피하기 위한 최소한의 방어선 구축이 필요합니다."
    ...
    ai_opinion = f"🤖 **StockMap AI {mode} 심층 진단**\n\n"
    ai_opinion += f"현재 종목은 {time_unit}봉 기준 **{'우상향 모멘텀' if macd > signal else '조정 및 매물 소화'}** 국면에 놓여 있습니다. "
    if near_sup: ai_opinion += f"특히 지지 가격(**{sup:,.{decimals}f}{currency}**) 부근에서 하방 경직성이 강하게 나타나고 있어 반등 확률이 높습니다. "
    
    comments['AI'] = f"{ai_opinion}\n\n🎯 **최종 전략:** {strategy} 현재 AI는 **{position}** 포지션을 권장합니다."
...
with st.sidebar:
    ...
    <div class="style-box">
    <b>🔍 분석 모드 가이드</b><br>
    • <b>단기</b>: 일봉의 미세 파동을 읽어 며칠 내 '반등 타점'을 포착.<br>
    • <b>장기</b>: <b>주봉(Weekly)</b> 단위로 거대 추세를 읽어 대세 흐름을 판별.
    </div>
    """, unsafe_allow_html=True)
...
if target_query:
    ...
    if raw_df.empty:
        ...
        # ==========================================
        # 🌟 모바일 줌 버튼 및 신규 상장주 대응 차트
        # ==========================================
        tab1, tab2 = st.tabs(["주가 차트", "수급 에너지"])
        
        # 줌 제어 버튼 UI
        z_col1, z_col2, z_col3, _ = st.columns([0.15, 0.15, 0.15, 0.55])
        if z_col1.button("확대 +"): st.session_state.zoom_level = max(0.2, st.session_state.zoom_level - 0.2)
        if z_col2.button("축소 -"): st.session_state.zoom_level = min(5.0, st.session_state.zoom_level + 0.2)
        if z_col3.button("초기화 ↺"): st.session_state.zoom_level = 1.0

        # 🌟 신규 상장주 대응: 데이터 시작점과 설정한 범위 중 더 늦은 쪽 선택
        data_start = chart_df.index[0]
        target_start = datetime.now() - timedelta(days=int(default_days * st.session_state.zoom_level))
        final_start = max(data_start, target_start)
        ...
        with tab1:
            ...
            fig.update_layout(height=450, margin=dict(t=10, b=10, l=0, r=0), dragmode='pan', hovermode='x unified', xaxis=dict(range=[final_start, datetime.now()], rangeslider=dict(visible=False)))
        ...
        with tab2:
            ...
            obv_fig.update_layout(height=350, margin=dict(t=10, b=10, l=0, r=0), dragmode='pan', xaxis=dict(range=[final_start, datetime.now()]))
