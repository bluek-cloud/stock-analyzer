        ...
        if raw_df.empty: st.error("⚠️ 데이터를 불러올 수 없습니다. 종목명/코드를 확인하거나, 잠시 후 다시 시도해 주세요. (데이터 서버 일시 장애 가능성)")
            ...
            if len(chart_df) < 5: 
                ...
                with tab_indicator:
                    ...
                    st.caption("💡 각 지표 버튼을 클릭하면 초보자도 쉽게 이해할 수 있는 상세 원리와 실전 해석 가이드가 표시됩니다.")
                    desc = {
                        "ADX 추세강도": """주가의 상승/하락 방향과 무관하게 **'추세 자체의 강도(쏠림의 세기)'**를 측정합니다.

• **20 미만**: 횡보/박스권  
• **25 이상**: 유의미한 추세 형성 구간""",
                        "상대 거래량": """직전 평균 거래량 대비 당일의 **'시장 관심 집중도'**를 측정합니다.

• **평소 대비 150% 이상**: 주요 변곡점 및 돌파 시도의 신호""",
                        "OBV 누적": """주가 변동과 함께 움직이는 **'거래량의 누적 흐름'**을 추적합니다.

• **주가와 OBV 방향의 불일치(다이버전스)**를 통해 추세 전환 가능성 포착""",
                        "RSI 강도": """주가의 최근 상승폭과 하락폭을 비교한 **'단기 가격 탄력성(과열/침체) 지표'**입니다.

• **70 이상**: 과매수 권역  
• **30 이하**: 과매도 권역""",
                        "MACD 흐름": """단기·장기 이동평균선의 수렴과 확산을 이용한 **'추세의 방향 및 모멘텀 전환 신호'**입니다.

• **골든/데드크로스 및 0선 기준선 돌파**로 추세 방향 파악""",
                        "ATR 변동성": """주식의 최근 하루 평균 움직임인 **'실질 가격 변동폭(원/달러)'**을 나타냅니다.

• **노이즈(일시적 출렁임)에 털리지 않는 객관적 손절/익절 폭 설정**에 활용"""
                    }
                    for label, key in [("ADX 추세강도", "ADX"), ("상대 거래량", "VOL"), ("OBV 누적", "OBV"), ("RSI 강도", "RSI"), ("MACD 흐름", "MACD"), ("ATR 변동성", "ATR")]:
                        cl, cv = st.columns([0.25, 0.75])
                        with cl.popover(label, use_container_width=True):
                            st.markdown(desc.get(label, '설명 없음'))
