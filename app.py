...
def parse_query(query):
    ...
    if query.isdigit() and len(query) == 6:
        ...
        if not matched.empty:
            ...
            return f"{name} ({query})", query, query, "원", 0, "KS11" # 코스피 비교
    matched = krx_df[krx_df['Name'] == query]
    if not matched.empty:
        code = matched.iloc[0]['Code']
        return f"{query} ({code})", code, query, "원", 0, "KS11"
    return f"{query} (해외)", query, query, "$", 2, "US500" # S&P 500 비교
...
def calculate_indicators(df):
    ...
    df['OBV'] = (df['Volume'] * ((delta > 0).astype(int) - (delta < 0).astype(int))).cumsum()
...
def generate_signal_and_comments(df, index_df, sup, res, currency, decimals):
    ...
    rsi, macd, signal, vol_ratio, atr, obv = float(latest['RSI']), float(latest['MACD']), float(latest['Signal']), float(latest['Vol_Ratio']), float(latest['ATR']), float(latest['OBV'])
    
    # 🌟 넥스트 레벨 1: 멀티 타임프레임 (주봉 추세 추출)
    ...
    # 🌟 넥스트 레벨 2: 시장 상대 강도 (최근 1개월 수익률 비교)
    ...
    if abs(close - sup) / sup < 0.05: ai_opinion += f"📍 **[핵심 타점]** 강력한 지지선(**{sup:,.{decimals}f}{currency}**) 부근에서 지지력을 테스트 중입니다. "
...
if target_query:
    ...
    if df.empty:
        ...
        with col2:
            with st.container(border=True):
                st.markdown("### 🔍 **지지/저항 가격**")
                st.write(f"🛡️ **지지:** {sup:,.{decimals}f} {currency} | 🚧 **저항:** {res:,.{decimals}f} {currency}")

        with st.expander("🔬 지표별 상세 분석 (용어 클릭)", expanded=True):
            for label, key in [("상대 거래량", "VOL"), ("RSI 강도", "RSI"), ("MACD 흐름", "MACD"), ("ATR 변동성", "ATR")]:
                c1, c2 = st.columns([0.2, 0.8])
                with c1.popover(label, use_container_width=True): st.info(f"**{label}** 설명...")
                c2.markdown(comments.get(key))
            st.divider()
        ...
        with tab1:
            ...
            fig.update_layout(height=450, margin=dict(t=10, b=10, l=0, r=0), dragmode='pan', xaxis=dict(range=[initial_start, datetime.now()], rangeslider=dict(visible=False)))
