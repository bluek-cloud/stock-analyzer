...
st.markdown("""
    ...
    .watchlist-btn { margin-bottom: 5px; }
    </style>
    """, unsafe_allow_html=True)

# ==========================================
# 2. 글로벌 매크로 패널 (Option 3 반영)
# ==========================================
...
def get_macro_data():
    try:
        ...
        vix = fdr.DataReader('^VIX').tail(2) # 야후 파이낸스 데이터
...
def generate_detailed_opinions(df, sup, res, currency, decimals, is_short_term, time_unit):
    ...
    ai_opinion = f"🤖 **AI {time_unit}봉 진단:** 현재 **{'상승 추세' if macd > signal else '조정 국면'}**에 있으며, "
...
# ==========================================
# 4. 사이드바 (Option 2: Watchlist 반영)
# ==========================================
with st.sidebar:
    st.header("🏢 My StockMap")
    analyze_mode = st.radio("투자 성향", ["단기 (6개월/일봉)", "장기 (2년/주봉)"])
    
    # 관심 종목 추가 기능
    st.subheader("⭐ Watchlist 추가")
    add_query = st.text_input("종목명/코드", key="add_stock")
    ...
    for i, stock in enumerate(st.session_state.watchlist):
        ...
        if c1.button(f"🔍 {stock['name']}", key=f"ws_{stock['code']}", use_container_width=True):
            target_query = stock['code']
        if c2.button("❌", key=f"del_{stock['code']}"):
    ...
    for item in st.session_state.recent_searches:
        if st.button(f"▪️ {item['display_name']}", key=f"rs_{item['code']}", use_container_width=True):
...
if target_query:
    ...
    if raw_df.empty:
        ...
        # 차트
        tab1, tab2 = st.tabs(["📉 주가 차트", "📊 수급 에너지(OBV)"])
        f_start = datetime.now() - timedelta(days=default_days)
        ...
        with tab1:
            ...
            fig.update_layout(height=450, margin=dict(t=10, b=10, l=0, r=0), dragmode='pan', xaxis=dict(range=[f_start, datetime.now()], rangeslider=dict(visible=False)))
        ...
        with tab2:
            if 'OBV' in chart_df.columns:
                ...
                obv_fig.update_layout(height=350, margin=dict(t=10, b=10, l=0, r=0), dragmode='pan', xaxis=dict(range=[f_start, datetime.now()]))
