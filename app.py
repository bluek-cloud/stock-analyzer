...
def generate_detailed_opinions(df, sup, res, currency, decimals, is_short_term, time_unit, q_score, weekly_bullish=None):
    ...
    near_sup = abs(close - sup) / (sup + 1e-10) <= 0.03 # 지지선 3% 이내 근접 여부
    ...
    if regime == "에너지 응축 (스퀴즈)":
        comments['RSI'] = f"RSI({rsi:.1f}): 볼린저 밴드 수축 국면이므로 RSI의 움직임이 매우 둔화되어 있습니다."
        comments['MACD'] = f"MACD({macd_diff:,.{decimals}f}): 이동평균선이 밀집하며 MACD도 0선에 완전히 수렴했습니다."
    elif regime == "횡보 박스":
        comments['RSI'] = f"RSI({rsi:.1f}): 횡보장에서는 RSI의 신뢰도가 가장 높습니다. " + ("박스권 하단 지지선 터치로 반등이 예상됩니다." if rsi <= 40 else "박스권 상단 저항선 도달로 조정이 예상됩니다." if rsi >= 60 else "방향성을 탐색 중입니다.")
        comments['MACD'] = f"MACD({macd_diff:,.{decimals}f}): 뚜렷한 추세가 부재한 박스권입니다."
    elif regime == "강세 추세":
        comments['RSI'] = f"RSI({rsi:.1f}): 강한 매수세로 단기 과열(70 이상) 상태이나 추세는 굳건합니다." if rsi >= 70 else f"RSI({rsi:.1f}): 안정적인 상승 탄력을 유지하고 있습니다."
        comments['MACD'] = f"MACD({macd_diff:,.{decimals}f}): 상승 모멘텀이 강하게 유지되고 있습니다."
    elif regime == "상승 조정": 
        comments['RSI'] = f"RSI({rsi:.1f}): 상승 추세 속에서 조정을 받으며 지표가 식어가고 있습니다. 40~50 지지 확인이 필요합니다."
        comments['MACD'] = f"MACD({macd_diff:,.{decimals}f}): 단기적으로 모멘텀이 둔화되었으나, 장기 상승 추세는 훼손되지 않았습니다."
    elif regime == "단기 약세 전환":
        comments['RSI'] = f"RSI({rsi:.1f}): 주가가 단기 지지선(20일선)을 이탈하며 투심이 빠르게 냉각되고 있습니다."
        comments['MACD'] = f"MACD({macd_diff:,.{decimals}f}): 지표가 시그널선을 하향 돌파하며 단기 매도 압력이 가중되고 있습니다."
    elif regime == "약세 추세":
        comments['RSI'] = f"RSI({rsi:.1f}): 약세장에서는 지표가 침체권에 머뭅니다. 섣부른 진입은 피해야 합니다."
        comments['MACD'] = f"MACD({macd_diff:,.{decimals}f}): 하락 모멘텀이 강하며, 추세 반전 신호가 아직 부족합니다."
    elif regime == "변동성 폭발":
        comments['RSI'] = f"RSI({rsi:.1f}): 변동성 폭발로 인해 투심이 한쪽으로 극단적으로 쏠리는 국면입니다."
        comments['MACD'] = f"MACD({macd_diff:,.{decimals}f}): 단기 모멘텀이 급격하게 방향성을 분출하고 있습니다."

    comments['VOL'] = f"상대 거래량이 평균 대비 **{vol_ratio:.0f}%** 수준입니다."
    comments['OBV'] = f"최근 수급(OBV)이 **{'상승(자금 유입)' if obv > simple_prev_obv else '하락(자금 이탈)'}** 중입니다."
    comments['ATR'] = f"예상되는 실질 변동폭(ATR)은 주당 평균 **{vol_pct:.1f}% ({atr:,.{decimals}f}{md_currency})** 수준입니다."

    if is_short_term:
        if is_falling_knife: pos, strategy = "🔷 투매 진행 중 (절대 관망)", "대량 거래를 동반한 치명적 급락 발생. 하락 진정 시까지 절대 관망하십시오."
        elif res == 0 and close > prev['Close']: pos, strategy = "🔴 신고가 랠리 (강력 홀딩)", "과거 매물대를 모두 뚫어낸 신고가 영역입니다. 수익을 극대화하십시오."
        elif regime == "에너지 응축 (스퀴즈)":
            if bullish_div: pos, strategy = "🔴 응축 구간 선취매", "에너지 응축 중 상승 다이버전스 포착. 상방 돌파 확률이 높습니다."
            else: pos, strategy = "⚖️ 방향성 대기 (관망)", "뚜렷한 방향성 분출 전까지 관망하십시오."
        elif regime == "횡보 박스":
            if box_pos <= 35 or bullish_div: pos, strategy = "🟠 박스권 하단 매수", "박스권 하단 지지 확인. 상단을 목표로 한 단기 스윙 전략이 유효합니다."
            elif box_pos >= 65: pos, strategy = "🔵 단기 박스권 상단 매도", "저항 돌파 수급이 부족합니다. 리스크 관리를 권장합니다."
            else: pos, strategy = "⚖️ 단기 관망", "박스권 중간 지대 위치. 뚜렷한 타점 도달 전까지 대기하십시오."
        elif regime in ["강세 추세", "상승 조정"]:
            if rsi <= 55 or bullish_div: pos, strategy = "🔴 추세 눌림목 적극 매수", "강한 상승 추세 속 건전한 눌림목 발생. 매수 타점입니다."
            elif rsi >= 75: pos, strategy = "🔵 분할 익절", "단기 과열권 진입. 수익 보호를 위해 일부 현금화를 권장합니다."
            else: pos, strategy = "🟠 추세 보유 (홀딩)", "우상향 흐름 진행 중. 지속 보유 전략이 유효합니다."
        elif regime == "단기 약세 전환":
            if near_sup or bullish_div: pos, strategy = "🟠 단기 기술적 반등 공략", "단기 지지선은 깨졌으나 구조적 지지선에 도달했습니다. 반등 신호를 노린 짧은 트레이딩이 가능합니다."
            else: pos, strategy = "⚖️ 단기 관망", "주가가 20일선을 깨고 밀리고 있습니다. 확실한 바닥을 다질 때까지 관망하십시오."
        elif regime == "약세 추세":
            if rsi <= 30 or bullish_div: pos, strategy = "🟠 단기 기술적 반등 공략", "극단적 과매도 및 다이버전스 발생. 기술적 반등 매매만 권장합니다."
            else: pos, strategy = "🔷 적극 매도 및 관망", "하락 추세가 지배적입니다. 현금 비중을 높여 관망하십시오."
        else: pos, strategy = "🔴 돌파 추세 추종", "새로운 추세 형성 초입으로 적극 추종이 유리합니다."
    else:
        if regime == "상승 조정" and (box_pos > 50 or obv < simple_prev_obv): pos, strategy = "⚖️ 장기 눌림목 대기", "하락세 진정 및 지지선 확인 전까지 보수적 관망을 권장합니다."
        elif regime == "단기 약세 전환": pos, strategy = "⚖️ 장기 지지선 테스트", "장기 상승추세의 마지노선인 60일선 지지력을 시험하는 구간입니다."
        elif regime in ["강세 추세", "변동성 폭발", "상승 조정"]: pos, strategy = "🔴 비중 확대 (장기)", "대세 상승장입니다. 장기적 시각에서 비중 확대 전략이 유효합니다."
        elif regime == "약세 추세": pos, strategy = "🔷 비중 축소 (장기)", "대세 하락장이 지속 중입니다. 주식 비중 축소를 권장합니다."
        else: pos, strategy = "⚖️ 장기 관망", "변곡점이거나 방향성이 불분명한 구간입니다. 확실한 추세 형성 시까지 관망하십시오."

    if pos in buy_list and q_score < 30: pos, strategy = "⚖️ 관망", f"매수 신호가 포착되었으나 퀀트 스코어({q_score}점)가 낮아 신뢰도가 떨어집니다."
    elif pos in sell_list and q_score > 70 and not is_falling_knife: pos, strategy = "⚖️ 관망", f"매도 신호가 포착되었으나 퀀트 스코어({q_score}점)가 양호하여 상충이 발생합니다."

    # 🌟 리포트 텍스트 정렬 및 개행 정상화
    ai_op = f"🤖 **StockMap AI {'단기 스윙' if is_short_term else '장기 가치투자'} 심층 진단 리포트**\n\n"
    ...
    if is_short_term and weekly_bullish is not None:
        ...
        if regime in ["강세 추세", "상승 조정"]: 
            ai_op += "• **장기 흐름:** 주봉 상승세가 굳건하여 일봉 수준에서도 강한 지지력을 보입니다.\n\n" if weekly_bullish else "• **장기 흐름:** 단기는 긍정적이나 주봉은 하락 추세이므로 눈높이를 낮추십시오.\n\n"
        elif regime == "단기 약세 전환":
            ai_op += "• **장기 흐름:** 단기 파동은 훼손되었으나 주봉 대세선은 깨지지 않았습니다.\n\n" if weekly_bullish else "• **장기 흐름:** 단기와 장기 대세 시그널이 동시에 하방 압력을 호소 중입니다.\n\n"
        else: 
            ai_op += "• **장기 흐름:** 장기 흐름에 동조화되어 에너지가 응축/분출되는 구간입니다.\n\n"
    
    ai_op += "💡 **[국면 맞춤형 통합 해석]**\n\n"
    if is_falling_knife: ai_op += "🚨 **[초고위험 투매 경보]** 현재 주가가 비정상적으로 급락 중인 '패닉셀' 구간입니다. 철저히 관망하십시오.\n\n"
    elif res == 0: ai_op += "✨ **[신고가 랠리 분석]** 완벽한 신고가 영역에 진입했습니다. 강력한 추세가 이어질 확률이 높습니다.\n\n"
    elif regime == "단기 약세 전환": ai_op += "• 생명선(20일선)이 무너지며 위험 단기 조정에 진입했습니다. 무조건적인 매수보단 관망 시점입니다.\n\n"
    elif regime == "강세 추세": ai_op += "• 매수세가 주도하는 강세장입니다. 추세 이탈 전까지 보유가 유리합니다.\n\n"
    else: ai_op += f"• 현재 국면({regime})에 따른 안정적인 대응이 필요한 구간입니다.\n\n"
    
    ai_op += "📊 **[수급 및 주요 레벨]**\n\n"
    ai_op += f"• **세력 수급:** 누적 수급(OBV)이 꾸준히 {'유입되며 긍정적' if obv > simple_prev_obv else '이탈하며 부정적'}인 모습입니다.\n\n"
    ...
    if close > prev_candle_close and close > ma20 and prev_candle_close <= prev_ma20 and not is_falling_knife:
        if vol_ratio < 80 or (latest_high - max(latest_open, close)) > body * 1.5:
            ai_op += "🚨 **[가짜 상승(Bull Trap) 주의]** 저항 돌파 거래량이 부족하거나 윗꼬리가 깁니다. 추격 매수를 자제하십시오.\n\n"
        ...
        if vol_ratio < 70 or (min(latest_open, close) - latest_low) > body * 1.5:
            ai_op += "🚨 **[가짜 하락(Bear Trap) 주의]** 지지를 이탈했으나 하단 방어 흔적이 보입니다. 일시적 충격일 수 있습니다.\n\n"

    ai_op += "📅 **[단기 실전 대응 시나리오 가이드]**\n\n"
    if res == 0: ai_op += "• **상방 추세 시나리오:** 저항 없는 신고가 상태입니다. 추세 훼손 전까지 수익 극대화.\n\n"
    else: ai_op += f"• **상방 돌파 시나리오:** 1차 저항선인 **{res:,.{decimals}f}{md_currency}** 강하게 돌파 시 매수 관점 접근.\n\n"
    ai_op += f"• **하방 방어 시나리오:** 핵심 지지선은 **{sup:,.{decimals}f}{md_currency}** 입니다. 이탈 시 즉각 리스크 관리.\n\n"
...
def scan_200_pullback(top_n=200):
    ...
    found_stocks = []
    progress_bar = st.progress(0, text="📡 우량주 데이터 스캔 중...")
    ...
    for i, (idx, row) in enumerate(target_stocks.iterrows()):
        ...
        try:
            ...
            latest, prev = df.iloc[-1], df.iloc[-2]
            if (df['MA200'].diff().tail(10) <= 0).any(): continue
            ...
            found_stocks.append({'종목명': row['Name'], '종목코드': row['Code'], '현재가': int(latest['Close']), '200일선': int(latest['MA200'])})
...
if app_menu == "📊 단일 종목 심층 분석":
    ...
    if st.session_state.target_query:
        ...
        if raw_df.empty: st.error("데이터를 찾을 수 없습니다.")
            ...
            if len(chart_df) < 2: st.warning("데이터가 부족합니다.")
            else:
    ...
    if st.button("🚀 스캐너 작동", type="primary", use_container_width=True):
        ...
        if not res_df.empty:
            ...
            res_df['현재가'] = res_df['현재가'].apply(lambda x: f"{x:,} 원")
